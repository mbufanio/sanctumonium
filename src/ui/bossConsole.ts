...
export class BossConsole {
  ...
  private scope(s: BossSession, animateIn: boolean): SVGSVGElement {
    ...
    // BEYOND it (they can't appear inside your radar net).
    const perimeterR = Math.min(33, 26 + sensors.length * 1.2);
    ...
    perim.setAttribute("fill", rgba(COLORS.coverage, 0.05));
    ...
    svg.append(perim);
    this.label(svg, cx, cy - perimeterR - 1.8, "YOUR COVERAGE", COLORS.coverage, 1.9);

    // Position helpers. The asset sits at the centre, the player's devices ring
    // it (spread by their real distance so a big roster doesn't collapse onto
    // one ring), and the boss drones sit in the OUTER band beyond the perimeter.
    const threatPos = (t: ThreatUnit) => polar(cx, cy, 37 + (t.distance / maxDist) * 9, t.bearing);
    const devPos = (d: DeviceUnit) => polar(cx, cy, 13 + ((d.distance ?? 2) / maxDevDist) * 13, d.bearing);
